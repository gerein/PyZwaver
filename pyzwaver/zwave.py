...
SERIALCMD_TO_CONTROLLERREQUEST_PARSE_TABLE = {
    ...
    0x03: ['B{deviceoptions}', 'B{generic}', 'B{specific}', 'A{nodeparm}'],   #  SERIAL_API_APPL_NODE_INFORMATION
    0x05: [],   #  ZW_GET_CONTROLLER_CAPABILITIES
    0x06: ['B{rxacktimeout}', 'B{rxbytetimeout}'],   #  SERIAL_API_SET_TIMEOUTS
    0x07: [],   #  SERIAL_API_GET_CAPABILITIES
    0x08: [],   #  SERIAL_API_SOFT_RESET
    0x12: ['B{node}', 'B{txOptions}'],   #  ZW_SEND_NODE_INFORMATION
    0x13: ['B{node}', 'SZCMD{command}', 'B{txOptions}', 'B{callback}'],   #  ZW_SEND_DATA
    0x15: [],   #  ZW_GET_VERSION
    0x1c: ['B{noRandomBytes}'],   #  ZW_GET_RANDOM
    0x20: [],   #  ZW_MEMORY_GET_ID
    0x23: ['W{offset}', 'B{length}'],   #  ZW_READ_MEMORY
    0x41: ['B{node}'],   #  ZW_GET_NODE_PROTOCOL_INFO
    0x42: [],   #  ZW_SET_DEFAULT
    0x48: ['B{node}'],   #  ZW_REQUEST_NODE_NEIGHBOR_UPDATE
    0x4a: ['B{mode}'],   #  ZW_ADD_NODE_TO_NETWORK
    0x4b: ['B{mode}'],   #  ZW_REMOVE_NODE_FROM_NETWORK
    0x4d: ['B{mode}'],   #  ZW_CONTROLLER_CHANGE
    0x50: ['B{mode}'],   #  ZW_SET_LEARN_MODE
    0x56: [],   #  ZW_GET_SUC_NODE_ID
    0x60: ['B{node}'],   #  ZW_REQUEST_NODE_INFO
    0x62: ['B{node}'],   #  ZW_IS_FAILED_NODE_ID
    0x80: ['B{node}', 'B{removebad}', 'B{removenonreps}', 'B{_mustbezero}'],   #  ZW_GET_ROUTING_INFO
    0xd0: ['B{state}'],   #  ZW_SET_PROMISCUOUS_MODE
}

SERIALCMD_TO_DEVICERESPONSE_PARSE_TABLE = {
    0x02: ['B{version}', 'B{capabilities}', 'A{nodemask}', 'B{chipType}', 'B{chipVersion}'],   #  SERIAL_API_GET_INIT_DATA
    0x05: ['B{retval}'],   #  ZW_GET_CONTROLLER_CAPABILITIES
    0x06: ['B{prevrxacktimeout}', 'B{prevrxbytetimeout}'],   #  SERIAL_API_SET_TIMEOUTS
    0x07: ['W{version}', 'W{manufacturer}', 'W{productType}', 'W{productId}', 'L{functionBitmask}'],   #  SERIAL_API_GET_CAPABILITIES
    0x12: ['B{retVal}'],   #  ZW_SEND_NODE_INFORMATION
    0x13: ['B{retval}'],   #  ZW_SEND_DATA
    0x15: ['L{rest}'],   #  ZW_GET_VERSION
    0x1c: ['B{randomGenerationSuccess}', 'a{randomBytes}'],   #  ZW_GET_RANDOM
    0x20: ['W{homeidMSW}', 'W{homeidLSW}', 'B{node}'],   #  ZW_MEMORY_GET_ID
    0x23: ['R{buffer}'],   #  ZW_READ_MEMORY
    0x41: ['B{capability}', 'B{security}', 'B{_reserved}', 'B{basic}', 'B{generic}', 'B{specific}'],   #  ZW_GET_NODE_PROTOCOL_INFO
    0x50: ['B{true}'],   #  ZW_SET_LEARN_MODE
    0x56: ['B{sucnodeid}'],   #  ZW_GET_SUC_NODE_ID
    0x60: ['B{retval}'],   #  ZW_REQUEST_NODE_INFO
    0x62: ['B{retval}'],   #  ZW_IS_FAILED_NODE_ID
    0x80: ['NDMASK{nodelist}'],   #  ZW_GET_ROUTING_INFO
}

SERIALCMD_TO_NODEREQUEST_PARSE_TABLE = {
    0x04: ['B{rxStatus}', 'B{node}', 'SZCMD{command}', 'b{rxRSSIVal}', 'b{securityKey}'],   #  APPLICATION_COMMAND_HANDLER
    0x12: ['B{callback}', 'B{txStatus}'],   #  ZW_SEND_NODE_INFORMATION
    0x13: ['B{callback}', 'B{txstatus}'],   #  ZW_SEND_DATA
    0x42: ['B{callback}'],   #  ZW_SET_DEFAULT
    0x48: ['B{callback}', 'B{status}'],   #  ZW_REQUEST_NODE_NEIGHBOR_UPDATE
    0x49: ['B{status}', 'L{data}'],   #  ZW_APPLICATION_UPDATE
    0x4a: ['B{callback}', 'B{status}', 'B{node}', 'L{rest}'],   #  ZW_ADD_NODE_TO_NETWORK
    0x4b: ['B{callback}', 'B{status}', 'B{node}', 'L{rest}'],   #  ZW_REMOVE_NODE_FROM_NETWORK
    0x4d: ['B{callback}', 'B{status}', 'B{node}', 'A{rest}'],   #  ZW_CONTROLLER_CHANGE
    0x50: ['B{callback}', 'B{status}', 'B{node}', 'A{nodeparm}'],   #  ZW_SET_LEARN_MODE
}
